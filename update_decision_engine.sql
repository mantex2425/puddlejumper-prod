 -- file: decision_engine_v2.sql
  -- Description: v2.7 Added "Towards Mode" - Accept rides heading towards target destination
  -- Usage: psql -d puddlejumper -f decision_engine_v2.sql

  -- Drop previous versions to avoid signature conflicts
  DROP FUNCTION IF EXISTS app_private.decision_engine_v2(text, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, text);
  DROP FUNCTION IF EXISTS app_private.decision_engine_v2(text, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, text);
  ...
  DECLARE
      ...
      v_settings               jsonb;
      v_calc_method            text;
      v_target_per_hour        numeric;
      v_target_per_mile        numeric;
      ...
      v_deadhead_rate_hr       numeric;
      v_deadhead_rate_mi       numeric;
  ...
  BEGIN
      ...
      IF v_settings IS NULL THEN
          RETURN QUERY SELECT 'ERROR'::text, 'No settings found'::text, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric;
          RETURN;
      END IF;

      -- 2. Extract Active Market
      ...
      v_target_per_hour  := COALESCE((v_settings->>'min_effective_hourly_rate')::numeric, 22.0);
      v_target_per_mile  := COALESCE((v_settings->>'min_effective_dollar_per_mile')::numeric, 1.67);
      ...
      -- 4. Red Zone Check (ALWAYS enforced, even in Towards Mode)
      IF v_red_zones IS NOT NULL AND jsonb_array_length(v_red_zones) > 0 THEN
          IF EXISTS (SELECT 1 FROM jsonb_array_elements_text(v_red_zones) r(hex) WHERE r.hex IN (v_pickup_hex, v_dropoff_hex)) THEN
              RETURN QUERY SELECT 'DECLINE'::text, 'Location in Red Zone'::text, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric;
              RETURN;
          END IF;
      END IF;

      -- 5. Pickup Distance Check (ALWAYS enforced, even in Towards Mode)
      IF pickup_miles_in > v_max_pickup_miles THEN
          RETURN QUERY SELECT 'DECLINE'::text, format('Pickup too far (%s mi)', pickup_miles_in)::text, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric, 0.0::numeric;
          RETURN;
      END IF;

      -- 6. TOWARDS MODE: Accept rides heading towards target
      IF towards_active AND towards_target_lat IS NOT NULL AND towards_target_lng IS NOT NULL THEN
          -- Calculate distances to target using PostGIS
          ...
          -- If dropoff is closer to target than pickup, we're heading towards it
          IF v_dropoff_to_target < v_pickup_to_target THEN
              RETURN QUERY SELECT
                  'ACCEPT'::text,
                  format('Towards mode: %s mi closer to target', round((v_pickup_to_target - v_dropoff_to_target)::numeric, 1))::text,
                  gross_payout_in::numeric,
              ...
              RETURN;
          END IF;
          -- If not heading towards target, fall through to normal logic
      END IF;

      -- 7. Deadhead / Return Trip (WITH EXCEPTION PROTECTION)
      ...
      -- 8. FINANCIALS
      v_deadhead_rate_mi := v_target_per_mile * v_deadhead_percent;
      v_deadhead_rate_hr := v_target_per_hour * v_deadhead_percent;

      IF v_calc_method = 'per_mile' THEN
          v_calculated_cost := v_return_miles * v_deadhead_rate_mi;
      ELSIF v_calc_method = 'per_hour' THEN
          v_calculated_cost := (v_return_minutes / 60.0) * v_deadhead_rate_hr;
      ELSE
          v_calculated_cost := GREATEST(v_return_miles * v_deadhead_rate_mi, (v_return_minutes / 60.0) * v_deadhead_rate_hr);
      END IF;
      ...
      -- 9. Verdict Logic
      IF v_calc_method IN ('per_hour', 'both') AND v_hourly_rate < v_target_per_hour THEN v_pass_hourly := FALSE; END IF;
      IF v_calc_method IN ('per_mile', 'both') AND v_dollars_per_mile < v_target_per_mile THEN v_pass_mileage := FALSE; END IF;

      IF v_pass_hourly AND v_pass_mileage THEN
          v_verdict := 'ACCEPT'; v_reason := 'Rates met';
      ELSE
          v_verdict := 'DECLINE';
