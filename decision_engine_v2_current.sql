-- 1. Drop old version to allow signature changes (if any internal types changed, though we kept return types same)
DROP FUNCTION IF EXISTS app_private.decision_engine_v2(
    text, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, 
    text, boolean, numeric, numeric, text, numeric, numeric, numeric, boolean
);

-- 2. Create Auto-Optimize Aware Version
CREATE OR REPLACE FUNCTION app_private.decision_engine_v2(
    ...
    market_id_in text DEFAULT NULL::text, 
    towards_active boolean DEFAULT false, 
    towards_target_lat numeric DEFAULT NULL::numeric, 
    towards_target_lng numeric DEFAULT NULL::numeric, 
    towards_market_id text DEFAULT NULL::text, 
    current_lat_in numeric DEFAULT NULL::numeric, 
    current_lng_in numeric DEFAULT NULL::numeric, 
...
DECLARE
    ...
    v_timezone               text;
    v_auto_optimize          boolean; -- NEW TOGGLE
...
BEGIN
    ...
    -- [TIMEZONE] Set Driver Timezone (Default to Chicago if missing)
    v_timezone              := COALESCE(v_settings->>'timezone', 'America/Chicago');
    
    -- [AUTO-OPTIMIZE] Check toggle (Default to TRUE)
    ...
    -- [SHIFT] DETERMINE ACTIVE SHIFT (Using Dynamic Timezone)
    ...
    IF towards_active AND towards_target_lat IS NOT NULL AND towards_target_lng IS NOT NULL THEN
        ...
        IF towards_market_id IS NOT NULL THEN
            SELECT elem INTO v_other_market FROM jsonb_array_elements(v_settings->'markets') elem WHERE (elem->>'id') = towards_market_id LIMIT 1;
            IF v_other_market IS NOT NULL THEN
                ...
                END IF;
            END IF;
        ELSIF towards_target_lat IS NOT NULL THEN
            v_target_hex := h3_latlng_to_cell(point(towards_target_lng::float8, towards_target_lat::float8), H3_RES);
        ...
        ELSIF v_net_progress > 0 AND v_backtrack_miles <= towards_backtrack_tolerance THEN
             ...
             IF v_efficiency < v_towards_efficiency_threshold THEN
                 v_verdict := 'DECLINE';
                 v_reason := format('Inefficient route: %s%% progress (Only %s mi gained on %s mi trip)', 
             ...
             ELSE
                 ...
                 IF v_overshoot_h3_dist > 0 THEN
                     v_overshoot_cost := v_overshoot_miles * COALESCE((v_settings->>'cost_per_mile')::numeric, 0.67) * v_deadhead_percent;
    ...
    ELSE
        ...
        ELSE
            ...
            IF NOT v_lands_in_other_market THEN
                -- LEAVING MARKET
                BEGIN
                    SELECT MIN(h3_grid_distance(v_dropoff_hex::h3index, g_hex::h3index)) INTO v_dist_to_green FROM unnest(v_local_green_zones) g_hex;
                    v_return_miles := COALESCE(v_dist_to_green, 50) * 0.4;
                EXCEPTION WHEN OTHERS THEN v_return_miles := 20.0; END;
