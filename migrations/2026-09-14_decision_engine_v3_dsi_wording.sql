-- 2026-09-14  decision_engine_v3: DSI wording carries its unit; display coherence.
--
-- Built from the LIVE function (pg_get_functiondef, md5(prosrc) 74dc76a4...), not
-- from migrations/2026-08-18_decision_engine_v3.sql, which is behind the live
-- engine (it lacks the 2026-08-25 pulse disable). This file is now the source.
--
-- WORDING AND DISPLAY ONLY. No verdict condition, threshold, formula, cost or gate
-- changes. Changes:
--   1. Reason text: 'DSI $13.20/hr — below your $14.00/hr bar' (net_hourly/indexed).
--      Drivers on the legacy weighted formula get 'Below your bar' / 'Clears your bar':
--      their score is an index, which must not be shown and is not DSI.
--   2. Returned net_hourly_usd is floored to the cent instead of rounded.
--   3. A mileage-floor decline is classed 'gate:mileage_floor' instead of 'threshold'.
-- Verified before apply: every offer this driver logged in the last 30 days returns
-- the identical verdict from the old and new function (see commit message).
...
DECLARE
    ...
    v_nearest_hex         text;
    v_dsi_text            text;      -- 'DSI $13.20/hr'; NULL for the legacy weighted formula
    v_bar_text            text;      -- '$14.00/hr'; NULL for the legacy weighted formula
    v_verdict             text;
    v_reason              text;
    v_trace               jsonb;
BEGIN
    ...
    ELSE
        -- WORDING ONLY (2026-09-14, "DSI = net $/hr" work order). The three
        -- conditions below are unchanged. DSI is net dollars per hour and is
        -- always written with its unit; the figure is FLOORED to the cent so
        -- the text can never show a declined offer at or above the bar
        -- (13.996 is "$13.99/hr", not "$14.00/hr"). A driver still on the
        -- legacy 'weighted' formula is scored on an index, not dollars. That
        -- index must never be shown to a driver, and it is not DSI, so their
        -- wording carries no number at all.
        IF v_dsi_formula <> 'weighted' THEN
            v_dsi_text := 'DSI ' || CASE WHEN v_dsi < 0 THEN '-' ELSE '' END || '$'
                          || to_char(abs(floor(v_dsi * 100) / 100), 'FM999990.00') || '/hr';
            v_bar_text := '$' || to_char(v_dsi_threshold, 'FM999990.00') || '/hr';
        END IF;
        ...
        IF v_dsi < v_dsi_threshold THEN
            v_verdict := 'DECLINE';
            v_reason  := CASE WHEN v_dsi_text IS NULL THEN 'Below your bar'
                              ELSE format('%s — below your %s bar', v_dsi_text, v_bar_text) END;
        ...
        ELSE
            v_verdict := 'ACCEPT';
            v_reason  := CASE
                WHEN v_return_source = 'dropoff_in_green' THEN format('%s, stays in market', coalesce(v_dsi_text, 'Clears your bar'))
                WHEN v_return_miles > 0 THEN format('%s after %s mi return',
                                                    coalesce(v_dsi_text, 'Clears your bar'), round(v_return_miles, 1))
                ELSE coalesce(v_dsi_text, 'Clears your bar')
            END;
        END IF;
    END IF;
