-- 2026-09-15  decision_engine_v3: the driver sees GROSS $/hr; the verdict stays DSI.
--
-- Based on 2026-09-15_decision_engine_v3_default_net_hourly.sql (live, md5(prosrc) 5eefa09f).
-- DISPLAY ONLY. No verdict condition, bar, formula, cost, gate or default changes.
--   - Section 11b computes the offer's gross $/hr (floored) and the gross this trip
--     needed to clear the bar (bar + committed mph x cost, rounded up), plus whether a
--     threshold decline was caused by the modeled return leg.
--   - Reason text is gross: "$27.03/hr — needs $29.35 on this trip" (gap >= 50c),
--     "$13.99/hr" (smaller gap), "$18.20/hr — pulls you out of market" (return leg),
--     accepts "$21.27/hr". Weighted-formula wording unchanged.
--   - trace_data gains grossHourlyShown, neededGrossHourly, needsShown,
--     netHourlyNoReturn, declineCause.
...
DECLARE
    ...
    v_decline_cause       text;      -- 'rate' | 'return_leg' on a threshold decline
...
BEGIN
    ...
    IF v_dsi_formula <> 'weighted' AND v_hourly_rate IS NOT NULL THEN
        v_gross_shown  := floor(v_hourly_rate * 100) / 100;
        v_gross_text   := '$' || to_char(v_gross_shown, 'FM999990.00') || '/hr';
        IF v_committed_mph IS NOT NULL THEN
            v_needed_gross := ceil((v_dsi_threshold + v_committed_mph * v_cost_per_mile) * 100) / 100;
        END IF;
    ...
    ELSE
        ...
        IF v_dsi < v_dsi_threshold THEN
            ...
            ELSIF v_net_no_return IS NOT NULL AND v_net_no_return >= v_dsi_threshold THEN
