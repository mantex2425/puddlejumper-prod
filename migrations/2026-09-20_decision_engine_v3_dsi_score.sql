...
DECLARE
    ...
    v_dsi_score           integer;   -- DSI: shown / needed * 100. 100 IS the accept line.
...
BEGIN
    ...
    ELSE
        ...
        ELSIF v_gross_floor > 0 AND v_gross_check IS NOT NULL AND v_gross_check < v_gross_floor THEN
            ...
            ELSE
                v_reason := v_gross_text;
            END IF;
        -- MILEAGE FLOOR RETIRED 2026-09-19. It declined an offer whose dollars-per-mile
        -- fell under a tapering required rate -- pre-DSI machinery, from before the bar
        -- and the gross minimum decided anything. It never once fired for Andrew (0 of
        -- 164 accepts in 30 days), because his basePerMile was 0.45; but a driver who
        -- never had a mileageFloorConfig fell back to min_effective_dollar_per_mile of
        -- 0.70, and 7 of those same 164 accepts would have died on it -- rides the engine
        -- judged worth taking, killed by a gate the driver never set and could not see.
        -- v_effective_per_mile is still computed and still reported in the trace, so the
        -- numbers stay observable; nothing reads it for a verdict.
        ELSE
            v_verdict := 'ACCEPT';
            v_reason  := CASE
    ...
    v_decline_class := CASE
        ...
        -- 'gate:mileage_floor' retired 2026-09-19 with the gate itself
        ELSE 'threshold'
    END;
    ...
    -- THE DSI THE DRIVER SEES (2026-09-20). What the offer pays over what THIS trip had to
    -- pay: 100 is the line, at or above clears, below is short. Both sides are the same
    -- object -- the on-screen gross per hour of the whole commitment -- so a driver can
    -- check the numerator against the offer card, and the minimum-on-screen rule needs no
    -- second threshold because it is already inside v_needed_gross.
    --
    -- Null on purpose when there is no line to measure against (no needed gross, or the
    -- 'weighted' formula, which has no honest $/hr). A gate -- red zone, pickup too far --
    -- returns before this point and carries no score at all: a veto is not a cheap hour.
    -- CLAMPED AT THE LINE. Plain rounding let four declines in 30 days print 100: a ratio
    -- of 99.6 rounds up, and a driver reading "100 clears" beside an amber tile learns the
    -- app cannot be trusted. 100 is reserved for offers that actually cleared, and 99 is
    -- the highest a decline can score.
    IF v_gross_shown IS NOT NULL AND v_needed_gross IS NOT NULL AND v_needed_gross > 0 THEN
        v_dsi_score := round(v_gross_shown / v_needed_gross * 100);
        IF v_gross_shown >= v_needed_gross THEN
            v_dsi_score := GREATEST(v_dsi_score, 100);
        ELSE
            v_dsi_score := LEAST(v_dsi_score, 99);
        END IF;
    END IF;
